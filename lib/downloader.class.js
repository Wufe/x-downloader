var Downloader = function(){
    ...
    var Chunk = function( parent, data, test){
    	var parent = parent;
    	...
    	var startDownload = function(){
    		var options = {
    			...
    			path: URI.path,
    			method: 'GET',
    			headers: ( content_length !== undefined ? { "range": range } : undefined )
    		};
            if(test != true ){
    ...
    this.download = function( URI, filename, test, callback, scope ){ // download( URI, [ filename, ] callback, scope )
    	URI = url.parse( URI );
    	...
    	this.getHeaders( URI, function( headers ){
			content_length = headers[ 'content-length' ] === undefined ? undefined : parseInt( headers[ 'content-length' ] );
			this.createDummy( filename, content_length, function(){
                ...
                if( content_length !== undefined ){
                    ...
                    for( var i = 0; i < thread_count; i++ ){
                        ...
                        var chunk = new Chunk( this, thread_obj, test );
    ...
    this.chunkToTest = function(data){
        return new Chunk(this, data, true)
    }
